...
then
    while [ $((PORT < ENDPORT)) != "0" ]; do
        PORT=$((PORT+1))
        echo "Stopping $PORT"
        redis-cli -p $PORT -h $CLUSTER_HOST shutdown nosave
    done
    exit 0
fi
...
then
    ...
    while [ 1 ]; do
        clear
        date
        redis-cli -p $PORT -h $CLUSTER_HOST cluster nodes | head -30
        sleep 1
    done
    exit 0
fi
...
then
    while [ $((PORT < ENDPORT)) != "0" ]; do
        PORT=$((PORT+1))
        redis-cli -p $PORT  -h $CLUSTER_HOST $2 $3 $4 $5 $6 $7 $8 $9
    done
    exit 0
fi
